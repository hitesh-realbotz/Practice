...
drop database student_portal2;
create database student_portal2;
show databases;
use student_portal2;
show tables;
...
join course_years cy on cy.id = sc.course_year_id
...
select s.first_name, c.course_name, cy.year_id as Year, sub.sub_name from students s
join selected_courses sc on sc.student_id = s.id
join course_years cy on cy.id = sc.course_year_id
join courses c on c.id = cy.course_id
...
select s.first_name, c.course_name, cy.year_id as Year,  sub.sub_name as Subject,  sub.marks as `Total Marks` from students s
join selected_courses sc on sc.student_id = s.id
join course_years cy on cy.id = sc.course_year_id
join courses c on c.id = cy.course_id
join year_subjects ysub on ysub.course_year_id = cy.id
...
select s.first_name, c.course_name, cy.year_id as Year,  sub.sub_name as Subject,(select marks from obtained_marks where student_id = s.id and year_subject_id = ysub.id) as `Obtained Marks`,  sub.marks as `Total Marks` from students s
join selected_courses sc on sc.student_id = s.id
join course_years cy on cy.id = sc.course_year_id
join courses c on c.id = cy.course_id
join year_subjects ysub on ysub.course_year_id = cy.id
...
select s.first_name, c.course_name, cy.year_id as Year,  sub.sub_name as Subject,om.marks as `Obtained Marks`,  sub.marks as `Total Marks` from students s
join selected_courses sc on sc.student_id = s.id
join course_years cy on cy.id = sc.course_year_id
join courses c on c.id = cy.course_id
join year_subjects ysub on ysub.course_year_id = cy.id
join subjects sub on sub.id = ysub.sub_id
join obtained_marks om on om.year_subject_id = ysub.id and om.student_id = s.id
order by s.id
;
