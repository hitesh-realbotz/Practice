export const PRODUCTS = "Products";
export const CART = "Cart";
export const PROFILE = "Profile";
export const DASHBOARD = "Dashboard";
export const ORDERS = "Orders";
export const LOGOUT = "Logout";
export const LOGIN = "Login";

export const S_QUESTION_1 = "First Mobile Brand?";
export const S_QUESTION_2 = "Birth Place?";
export const S_QUESTION_3 = "Favorite Dish?";

export const SHIPPING_METHOD_1 = "Standard";
export const SHIPPING_METHOD_2 = "By-air";

export const EMAIL_ERROR_TAG = "emailError";
export const S_QUESTION_ERROR_TAG = "securityQuestionError";
export const ANSWER_ERROR_TAG = "answerError";

export const BUYER_NAME_ERROR_TAG = "buyerNameError";
export const CONTACT_NO_ERROR_TAG = "contactNoError";
export const ADDRESS_ERROR_TAG = "addressError";
export const AMOUNT_ERROR_TAG = "amountError";
export const CARD_NO_ERROR_TAG = "cardNoError";
export const CVV_ERROR_TAG = "cvvError";
export const PIN_ERROR_TAG = "pinError";
export const SHIPPING_METHOD_ERROR_TAG = "shippingMethodError";

export const EMAIL_VERIFICATION = "Email";
export const ANSWER_VERIFICATION = "Answer";
export const RESET_PASSWORD = "Password";



export const REGEX_EMAIL = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
export const REGEX_DIGIT_ONLY = /^\d+$/;


export const ITEMS_PER_PAGE = 8;

export const SORT_ORDER_ASC = "Ascending";
  export const SORT_ORDER_DESC = "Descending";

export const SORT_BY_TITLE = "title";
export const SORT_BY_TITLE_LABEL = "Title";
export const SORT_BY_PRICE = "price";
export const SORT_BY_PRICE_LABEL = "Price";
export const SORT_BY_CATEGORY = "category";
export const SORT_BY_CATEGORY_LABEL = "Category";
export const SORT_BY_BRAND = "brand";
export const SORT_BY_BRAND_LABEL = "Brand";
export const FOR_PRODUCT = "Product";

export const REMOVE_CART_ITEM_TITLE = "Are you sure to remove item?";
export const DECREASE_CART_QTY_TITLE = "Are you sure to reduce quantity?";
export const CLEAR_CART_TITLE = "Are you sure to clear cart?";




